...
// Simulated server URL (JSONPlaceholder, but we only fake POST/GET)
...
function addQuote() {
  ...
  if (text && category) {
    ...
    saveQuotes();
    syncWithServer(newQuote); // push new quote to server
    document.getElementById("newQuoteText").value = "";
    document.getElementById("newQuoteCategory").value = "";
    alert("Quote added!");
    filterQuotes();
...
async function syncWithServer(newQuote = null) {
  try {
    // 1. Fetch latest server data
    const response = await fetch(SERVER_URL);
    let serverQuotes = await response.json();

    // (Simulated: map JSONPlaceholder posts -> quotes)
    ...
    serverMap.forEach((srvQuote, id) => {
      if (!localMap.has(id)) {
        quotes.push(srvQuote); // add missing
      } else {
        localMap.set(id, srvQuote); // overwrite local with server
      }
    });

    saveQuotes();
    filterQuotes();
    showNotification("Quotes synced with server. Conflicts resolved (server wins).");

    // 3. Push new quote to server (simulation only)
    if (newQuote) {
